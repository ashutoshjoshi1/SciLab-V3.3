...
# Sightly adjusted for python 2 compatibility.
...
try:
    ...
    long = int  # Python 3 fallback

class LongIntEnum(long, Enum):
    """Enum where members are also (and must be) longs"""
    def __new__(cls, value, *args):
        # Ensure the value is treated as long to avoid the int overflow issue
...
if __platform_system == 'Windows':
    ...
    # Build a list of candidate locations to support multiple deployment layouts.
    # Priority order:
    # 1) explicit env override
    # 2) historical packaged app path (legacy Blick layout)
    # 3) local SDK folder near this module
    # 4) system install (System32)
    # 5) PATH-based lookup by name
    ...
    # Prefer local explicit dcamapi.dll files near the code/workspace.
    ...
    if __dll is None:
        ...
        raise FileNotFoundError(
            "Unable to load '{}' (or dependencies). Set DCAMAPI_DLL_PATH or install DCAM API in System32. "
...
class DCAMPROP_OPTION(LongIntEnum):
    """
    options
    Note: changed IntEnum by LongIntEnum for python 2.x 32bits compatibility (very long integers here)
    """
